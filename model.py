...
def _ratings_train_and_test(use_headers, delimiter, input_file):
  ...
  Returns:
    ...
    sparse coo_matrix for test
  """
  headers = ['UserID', 'WindowID', 'Split', 'Sequence', 'JobTitle']
  ...
  ratings_df = pd.read_csv(input_file,
                           ...
                           dtype={
                               ...
                               'Split': np.int32,
                               'Sequence': np.int32,
                                'JobTitle': np.dtype('U50')
                           })

  ratings_df['rating'] = pd.Series(1,index=ratings_df.index)
  np_users = ratings_df.UserID.as_matrix()
  np_items = ratings_df.JobTitle.as_matrix()
  ...
  if n_users != max_user or n_items != max_item:
    ...
    ratings = ratings_df.as_matrix(['UserID', 'JobTitle', 'rating'])
